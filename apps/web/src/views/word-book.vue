  ...
  <div class="max-w-300 mx-auto mt-10 bg-linear-to-br bg-card-background rounded-lg p-10 shadow-lg">
    ...
    <div>
      <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        <div
            v-for="item in queryResult?.list"
            :key="item.id"
            class="min-h-50 rounded-lg p-4 border-blue-200 dark:border-blue-600/40 border shadow-md space-y-1 overflow-hidden"
        >
          ...
          <div class="text-sm text-gray-600 dark:text-gray-400 truncate-2">
            {{ item.definition }}
          </div>
      ...
      <UPagination
          ...
          show-edges
          :sibling-count="1"
          :total="queryResult?.total ? queryResult.total / 12 : 0"
      />
    </div>
  </div>
</template>

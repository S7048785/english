...
export class UserService {
  ...
  async login(user: UserLogin) {
    ...
    // 2. 验证密码（与存储的哈希比较）
    const isPasswordValid = await this.cryptoService.verifyPassword(plainPassword, userDb.password);
    if (!isPasswordValid) throw new HttpException("用户名或密码错误", HttpStatus.BAD_REQUEST);
